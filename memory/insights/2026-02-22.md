...
### 19:20 — Weapon slots return nil from GetEquippedItem (unresolved)
All variants of slot names (MeleeMainHand, Melee Main Hand, MainHand, etc.) return ok=true item=nil.
Weapons ARE equipped visually (Throkk has Speedy Reply). Root cause unknown — may be weapon-set system,
may need different approach (InventoryOwner component, or different Osiris DB).
SKIP for now — armor/accessory slots all work, that's 80% of the value.
