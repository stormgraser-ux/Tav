-- ============================================================
-- TavSync — Console-loadable version (crossplay-safe)
-- Load after entering a save: F11 → paste this one-liner:
--   load(Ext.IO.LoadFile("tavsync.lua"))()
-- Then type !sync in console to manually sync, or just save.
-- ============================================================

-- Guard against double-loading
if _TavSyncLoaded then
  _P("[TavSync] Already loaded — skipping")
  return
end
_TavSyncLoaded = true
...
-- Replaces the F6 hotkey (which required a registered mod)

Ext.RegisterConsoleCommand("sync", function()
  dumpParty()
end)
...
_P("[TavSync] Loaded (console mode — crossplay safe)")
_P("[TavSync] Type !sync to sync party, or just save the game")
